...
The V2 tokenizer is `PhysicalFilterbankTokenizer` (see its docstring). Its key
hyperparameters and the physics that pins each value are collected below so they
are easy to find and hard to second-guess. `model/config.py` (SMALL_DEEP_FB_CONFIG)
is the per-run OVERRIDE surface; these are the defaults + the justification.
"""
