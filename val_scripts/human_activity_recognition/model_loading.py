    ...
    if 'config' in hp:
        ...
        return {
            'encoder': {
                ...
                # PHz-Filterbank tokenizer hyperparameters (save/load chain-of-custody)
                'n_bands': cfg.get('n_bands', 32),
                'f_min': cfg.get('f_min', 0.3),
                'f_max': cfg.get('f_max', 15.0),
                'tokenizer_Q': cfg.get('tokenizer_Q', 4.0),
                'dft_size': cfg.get('dft_size', 512),
                'nyquist_margin': cfg.get('nyquist_margin', 0.9),
                'tokenizer_learnable': cfg.get('tokenizer_learnable', False),
                'tokenizer_norm': cfg.get('tokenizer_norm', 'frozen'),
                'use_amplitude': cfg.get('use_amplitude', True),
                'use_resolution_mask': cfg.get('use_resolution_mask', True),
                'use_rope': cfg.get('use_rope', False),
                'rope_min_period': cfg.get('rope_min_period', 1.0),
                'rope_max_period': cfg.get('rope_max_period', 1000.0),
            },
            'semantic_head': {
...
def load_model(
    ...
    encoder = IMUActivityRecognitionEncoder(
        ...
        n_bands=enc_cfg.get('n_bands', 32),
        f_min=enc_cfg.get('f_min', 0.3),
        f_max=enc_cfg.get('f_max', 15.0),
        tokenizer_Q=enc_cfg.get('tokenizer_Q', 4.0),
        dft_size=enc_cfg.get('dft_size', 512),
        nyquist_margin=enc_cfg.get('nyquist_margin', 0.9),
        tokenizer_learnable=enc_cfg.get('tokenizer_learnable', False),
        tokenizer_norm=enc_cfg.get('tokenizer_norm', 'frozen'),
        use_amplitude=enc_cfg.get('use_amplitude', True),
        use_resolution_mask=enc_cfg.get('use_resolution_mask', True),
        use_rope=enc_cfg.get('use_rope', False),
        rope_min_period=enc_cfg.get('rope_min_period', 1.0),
        rope_max_period=enc_cfg.get('rope_max_period', 1000.0),
    )
