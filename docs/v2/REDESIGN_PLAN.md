...
- One run with M1–M3. Evaluate with Milestone-0 harness: ZS-XD macro-F1 on the 5 main + 2 severe-OOD sets; rate-flatness; frame-F1/boundary; ablation grid.
...
8. **Open question ⚑ (human):** which 3 showcase scenarios and which single blessed label vocabulary ship in the default checkpoint's model card? Recommended: scenarios 1/2/3 above; label bank = union of the 5 main-test native vocabularies plus a documented open-vocab path.
