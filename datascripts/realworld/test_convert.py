"""
Tests for RealWorld HAR dataset conversion.

Run: python datascripts/realworld/test_convert.py
"""
...
def test_manifest_structure():
    ...
    # Check at least accelerometer channels exist
    for expected in ["acc_x", "acc_y", "acc_z"]:
...
def test_labels_mapping():
    ...
    print(f"  ✓ Found {len(activities)} unique activities: {sorted(activities)}")
    return True
...
def test_session_parquet_format():
    ...
    for session_dir in session_dirs[:5]:
        ...
        assert df["timestamp_sec"].is_monotonic_increasing, "Timestamps should be monotonically increasing"
...
def test_channel_statistics():
    ...
    for channel, values in all_stats.items():
        if values:
            ...
            # Accelerometer typically in range [-20, 20] m/s²
            if "acc" in channel:
                assert abs(mean) < 50, f"{channel} mean {mean} seems unreasonable"

            # Gyroscope typically in range [-10, 10] rad/s
            if "gyro" in channel:
                assert abs(mean) < 20, f"{channel} mean {mean} seems unreasonable"

    print("  ✓ Channel statistics look reasonable")
    return True
...
def run_all_tests():
    """Run all tests."""
    print("=" * 80)
    print("RealWorld HAR Dataset Conversion Tests")
    print("=" * 80)

    if not OUTPUT_DIR.exists():
        print(f"\n✗ Output directory not found: {OUTPUT_DIR}")
        print("Run conversion first: python datascripts/realworld/convert.py")
        return False
