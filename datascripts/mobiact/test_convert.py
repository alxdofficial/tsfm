"""
Tests for MobiAct dataset conversion.

Run: python datascripts/mobiact/test_convert.py
"""
...
def test_labels_mapping():
    ...
    print(f"  ✓ Found {len(activities)} unique activities: {sorted(activities)[:5]}...")
    return True
    ...
    for session_dir in session_dirs[:20]:
        ...
        for channel in EXPECTED_CHANNELS:
            if channel in df.columns:
                ...
                if len(values) > 0:
                    all_stats[channel].extend(values[:100])  # Sample 100 points
...
def run_all_tests():
    """Run all tests."""
    print("=" * 80)
    print("MobiAct Dataset Conversion Tests")
    print("=" * 80)

    if not OUTPUT_DIR.exists():
        print(f"\n✗ Output directory not found: {OUTPUT_DIR}")
        print("Run conversion first: python datascripts/mobiact/convert.py")
        return False
