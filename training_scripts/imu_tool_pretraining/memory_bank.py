...
class MemoryBank:
    ...
    def update(self, imu_emb: torch.Tensor, labels: List[str]):
        """
    ...
    def get_queue_embeddings(self, label_encoder, device: torch.device):
        """
        Get queue embeddings for loss computation.

        Encodes queue labels on-the-fly to generate text embeddings.
        This is efficient because:
        1. Text encoder is frozen (no gradients)
        2. Encoding is fast (~20ms for 4096 labels)
        3. Saves GPU memory (don't store text embeddings)

        Args:
            label_encoder: Label bank with encode() method
        ...
        active_labels = self.label_queue[:actual_size]
        active_imu = self.imu_queue[:actual_size]

        # Encode labels to get text embeddings
        text_emb_queue = label_encoder.encode(active_labels, normalize=True)

        # Handle both numpy arrays and tensors
        if not isinstance(text_emb_queue, torch.Tensor):
            text_emb_queue = torch.from_numpy(text_emb_queue)
        text_emb_queue = text_emb_queue.to(device)

        # Move IMU embeddings to device
        imu_queue_device = active_imu.to(device)

        return imu_queue_device, text_emb_queue
